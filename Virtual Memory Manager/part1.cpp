...
signed char backStoreValue[page_size];  // the backStoreValue containing reads from backing store
...
void handlePageFaults(int pageNumber);
...
void printPage(int logicalAddress){
    ...
    if(frameNumber == -1){
        ...
        if(frameNumber == -1){                      // if the page is not found in those contents
            handlePageFaults(pageNumber);           // page fault, call to handlePageFaults to get the frame into physical memory and the page table
...
// main opens necessary files and calls on printPage for every entry in the addresses file
